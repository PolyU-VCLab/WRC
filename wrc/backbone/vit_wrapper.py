...
def get_patch_size_channels(backbone_name):
    if "vit_small" in backbone_name:
        ...
        if backbone_name == "vit_base_patch16_clip_384":
            feats = 384
        elif backbone_name == "vit_base_patch16_clip":
            feats = 512
...
class PretrainedViTWrapper(nn.Module):
    ...
    def create_model(self, name: str, **kwargs) -> Tuple[VisionTransformer, transforms.Compose]:
        if self.name == 'vit_base_patch16_clip':
            ...
            model = timm.create_model(
                ...
                pretrained_cfg_overlay=dict(file=f'your/local/path/to/model/{name}/model.safetensors'),
                **kwargs,
            )
    ...
    def forward(
        ...
        if self.name == 'vit_base_patch16_clip':
            feats = self.model(x)
            cls_token = None
        elif self.name in ["vit_base_patch16_siglip_512.v2_webli"]:
