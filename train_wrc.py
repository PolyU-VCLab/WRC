...
from wrc.utils.training import get_batch, get_dataloaders, logger
from wrc.loss import Cosine_MSE
...
def trainer(cfg: DictConfig):
    ...
    if getattr(cfg, "model_ckpt", None) is not None:
        ...
        if os.path.exists(run_dir / ".hydra" / "config.yaml"):
            log_print(f"Loading WRC from {cfg.model_ckpt}, original run dir: {run_dir}")
            old_cfg = OmegaConf.load(run_dir / ".hydra" / "config.yaml")
            wrc = instantiate(old_cfg.model, _convert_="partial")  # or _recursive_=False if needed
        state = torch.load(cfg.model_ckpt, map_location="cpu", weights_only=False)
        state = state['wrc']
        wrc.load_state_dict(state["state_dict"] if "state_dict" in state else state)
        log_print(f"Loaded WRC from {cfg.model_ckpt}")
    ...
    if cfg.backbone.name == 'vit_base_patch16_clip':
    ...
    all_params = []
    all_params.extend(list(wrc.parameters()))
    print("Number of parameters: ", sum(p.numel() for p in all_params))
    optimizer_wrc = instantiate(cfg.optimizer, params=all_params)
    ...
    for epoch in range(epoch_start, cfg.epochs):
        ...
        for batch_idx, batch in enumerate(
            ...
            with autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # predict
                wrc_hr_feats = wrc(guidance_image_batch, lr_feats)

                # needed for downsampling regularization
                wrc_output = wrc_hr_feats

                if wrc_hr_feats.shape[0] < hr_feats.shape[0]:
                    repeat = hr_feats.shape[0] // wrc_hr_feats.shape[0]
                    wrc_hr_feats = wrc_hr_feats.repeat_interleave(repeat, dim=0)

                # optional crop slicing
                if "guidance_crop" in batch:
                    # guidance_crop: (N,2) in patch units (row, col)
                    wrc_hr_feats = torch.stack([
                        wrc_hr_feats[i, :, x_i: x_i + hr_feats.shape[-2], y_i: y_i + hr_feats.shape[-1]]
                        for i, (x_i, y_i) in enumerate(batch["guidance_crop"])
                    ], dim=0)

                # ============ Loss WRC ============ #
                loss = {"wrc_hr": 0.0, "wrc_reg": 0.0, "wrc_down": 0.0}

                # local loss
                loss["wrc_hr"] = criterion(wrc_hr_feats, hr_feats)["total"]
                ...
                # global loss
                with torch.no_grad():
                    tmp_feats, _ = backbone(F.interpolate(lr_image_batch, scale_factor=2, mode='bilinear', align_corners=True))
                loss["wrc_down"] = criterion(tmp_feats, wrc_output)["total"]

                # augment loss
                noised_guidance_image = batch.get("augmented_guidance_image")
                pred_uhr_feats = wrc(noised_guidance_image, feats)
                with torch.no_grad():
                    target_uhr_feats = wrc(guidance_image_batch, feats)

                loss["wrc_reg"] = criterion(pred_uhr_feats, target_uhr_feats.detach())["total"]


            loss_wrc = loss["wrc_hr"] + loss["wrc_down"] + loss["wrc_reg"]

            optimizer_wrc.zero_grad()
            loss_wrc.backward()
            optimizer_wrc.step()
            ...
            if batch_idx % FREQ == 0:
                ...
                writer.add_scalar(
                    "Learning Rate WRC",
                    optimizer_wrc.param_groups[0]["lr"],
                    current_step,
                )
            ...
            if (current_step % checkpoint_interval == 0) or (current_step >= cfg.max_steps):
                ...
                save_dict = {
                    "optimizer_wrc": optimizer_wrc.state_dict(),
                    "epoch": epoch,
                    "cfg": cfg,
                    "wrc": wrc.state_dict(),
                }
