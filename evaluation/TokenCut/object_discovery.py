...
def ncut(feats, dims, scales, init_image_size, tau = 0, eps=1e-5, im_name='', no_binary_graph=False, has_cls=True):
    ...
    A = (feats @ feats.transpose(1,0)).to(device)

    if no_binary_graph:
        A[A<tau] = eps
    else:
        A = torch.where(A > tau, 1.0, eps)
    D = torch.diag(torch.sum(A, dim=1)).to(device)
  
    # Print second and third smallest eigenvector 
    # _, eigenvectors = eigh(D-A, D, subset_by_index=[1,2])
    # eigenvec = np.copy(eigenvectors[:, 0])
    vals, eigenvectors = torch.lobpcg(D - A, B=D, k=3, largest=False)
    eigenvec = eigenvectors[:, 1].cpu().numpy()
