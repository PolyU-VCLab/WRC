...
def plot_image_label_prediction(image, bilinear_feats, pred_feats, label, pred_prob, backbone, save_dir=None):
    ...
    # Prediction
    plt.subplot(1, 3, 3)
    num_classes = int(pred_prob.max().item()) + 1 
    pred_one_hot = F.one_hot(pred_prob.long(), num_classes=num_classes).permute(0, 3, 1, 2).float()
    pred_logits_high_res = F.interpolate(
        pred_one_hot, 
        size=(448, 448), 
        mode='bicubic', 
        align_corners=False
    )
    pred_prob = torch.argmax(pred_logits_high_res, dim=1).squeeze(0)
